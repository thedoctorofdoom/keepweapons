...
This is a **ZScript-based addon** for [Project Brutality 0.4.1](https://github.com/pa1nki113r/Project_Brutality/tree/master), a total-conversion mod for classic Doom running on the **UZDoom/GZDoom** source ports.
...
└── zscript/
    └── Weapons/
        ...
        └── BaseWeapon_Functions.zsc        # extend class PB_WeaponBase — utility functions (overrides PB upstream)
```
...
These files are **full overrides** of PB upstream files from the `0.4.1` release (master branch). If PB updates `BaseWeapon.zc` or `BaseWeapon_Functions.zsc`, this addon must be rebased:

1. Download the upstream files via raw HTTP (e.g., `Invoke-WebRequest` or `curl`) and diff against addon files — do **not** rely on web-rendered content, as angle-bracket type parameters like `class<Ammo>` get silently stripped as HTML tags.
2. Re-apply the three CVar-guarded modifications listed above.
3. Preserve all other upstream changes verbatim — including new functions, updated signatures, and particle/visual changes.
...
- Load order matters: this addon must load **after** Project Brutality.
...
- **Project Brutality (0.4.1 / master)**: <https://github.com/pa1nki113r/Project_Brutality>
